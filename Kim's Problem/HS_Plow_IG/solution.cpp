...
void play(int l,int r){
	...
	if(mx-l < r-mx){
		for(int i=l;i<mx;++i){
			ans[a[mx]]=(ans[a[mx]] + (upper_bound(pos[a[i]].begin(),pos[a[i]].end(),r)-upper_bound(pos[a[i]].begin(),pos[a[i]].end(),mx))*b[a[i]] )%md;
		}
	}
	else{
		for(int i=mx+1;i<=r;++i){
			ans[a[mx]]=(ans[a[mx]] + (lower_bound(pos[a[i]].begin(),pos[a[i]].end(),mx)-lower_bound(pos[a[i]].begin(),pos[a[i]].end(),l))*b[a[i]] )%md;
		}
	}
